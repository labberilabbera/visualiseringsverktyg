...
export async function GET(_req: NextRequest, { params }: { params: { id: string; uploadId: string } }) {
  ...
  const res = await db.query(
    "SELECT data, mimetype, ai_image, model3d_url FROM uploads WHERE id = $1 AND project_id = $2",
    [params.uploadId, params.id]
  );
  if (!res.rows[0]) return NextResponse.json({ error: "Inte hittad" }, { status: 404 });
  return NextResponse.json({ data: res.rows[0].data, mimetype: res.rows[0].mimetype, aiImage: res.rows[0].ai_image, model3dUrl: res.rows[0].model3d_url });
}

export async function PATCH(req: NextRequest, { params }: { params: { id: string; uploadId: string } }) {
  await initDb();
  const db = getPool();
  const { aiImage, model3dUrl } = await req.json();
