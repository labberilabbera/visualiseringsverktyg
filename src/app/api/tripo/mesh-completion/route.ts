...
export async function POST(req: NextRequest) {
  ...
  try {
    const { segTaskId, partNames } = await req.json();
    if (!segTaskId) return NextResponse.json({ error: "missing_segTaskId" }, { status: 400 });
    const body: any = { type: "mesh_completion", original_model_task_id: segTaskId };
    if (partNames && partNames.length > 0) body.part_names = partNames;
