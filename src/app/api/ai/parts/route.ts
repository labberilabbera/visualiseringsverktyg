...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await model.generateContent([
      ...
      {
        text: "Look at this object in the image. List the main visual parts/components of this specific object that could be separately styled or textured. Return ONLY a JSON array of short Swedish part names (max 6 parts, 1-3 words each). Example for a car: ["Kaross","Hjul","Fönster","Grill","Innertak"]. Example for a chair: ["Sits","Ryggstöd","Ben","Armstöd"]. Return only the JSON array, nothing else."
      }
    ]);
