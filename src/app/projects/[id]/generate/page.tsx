...
export default function GeneratePage() {
  ...
  async function loadUploads(){
    ...
    setUploads(data.map((u:any)=>({
      id:u.id, filename:u.filename, mimetype:u.mimetype,
      aiImage:u.ai_image||undefined,
      model3d:u.model3d_url||undefined,
      genState:(u.ai_image?"done":"idle") as GenState,
      tripoState:u.model3d_url?"done":undefined,
    })));
    setReady(true);
  }
  ...
  async function generateOne(i:number){
    ...
    try{
      const dr=await fetch("/api/projects/"+projectId+"/uploads/"+u.id+"/data");
      const{data,mimetype}=await dr.json();
      ...
      if(json.images?.[0]){
        const aiImage=json.images[0];
        // Save to DB
        await fetch("/api/projects/"+projectId+"/uploads/"+u.id+"/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({aiImage})});
        setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,aiImage,genState:"done"}:x));
        setTab("ai");
    ...
    try{
      ...
      for(let a=0;a<60;a++){
        ...
        if(pd.status==="success"&&pd.modelUrl){
          // Save to DB
          await fetch("/api/projects/"+projectId+"/uploads/"+u.id+"/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({model3dUrl:pd.modelUrl})});
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px"}}>
          {uploads.map((u,i)=>(<div key={u.id} onClick={()=>setSelected(i)} style={{marginBottom:"8px",borderRadius:"8px",overflow:"hidden",border:selected===i?"2px solid #1a56db":"1px solid var(--border)",cursor:"pointer",background:"var(--bg)"}}>
            ...
            <div style={{display:"flex",gap:"3px",padding:"4px",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:"9px",padding:"2px 5px",borderRadius:"4px",fontWeight:500,background:u.model3d?"#7c3aed22":u.genState==="done"?"#16a34a22":u.genState==="loading"?"#1a56db22":u.genState==="error"?"#c0392b22":"#33333344",color:u.model3d?"#a78bfa":u.genState==="done"?"#22c55e":u.genState==="loading"?"#6ea8fe":u.genState==="error"?"#ef4444":"var(--text2)"}}>{u.tripoState==="loading"?(u.tripoProgress?u.tripoProgress+"%":"3D..."):u.model3d?"3D klar":u.genState==="done"?"klar":u.genState==="loading"?"...":u.genState==="error"?"fel":"väntar"}</span>
...
function ModelViewer({modelUrl}:{modelUrl:string}){
  ...
  useEffect(()=>{
    ...
    const build=()=>{
      ...
      mv.setAttribute("src",modelUrl);mv.setAttribute("alt","3D");
    ...
    if(customElements.get("model-viewer")){build();}
      ...
      setTimeout(build,3000); // fallback
    }
