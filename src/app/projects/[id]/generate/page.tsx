    ...
    <aside style={{width:"200px",minWidth:"200px",borderRight:"1px solid var(--border)",background:"var(--bg2)",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"8px"}}>{uploads.map((u,i)=>(<div key={u.id} onClick={()=>setSelected(i)} style={{marginBottom:"8px",borderRadius:"8px",overflow:"hidden",border:selected===i?"2px solid #1a56db":"1px solid var(--border)",cursor:"pointer",background:"var(--bg)"}}>
        ...
        <div style={{display:"flex",gap:"3px",padding:"4px",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:"9px",padding:"2px 5px",borderRadius:"4px",fontWeight:500,background:u.segmentedModel?"#f59e0b22":u.model3d?"#7c3aed22":u.genState==="done"?"#16a34a22":u.genState==="loading"?"#1a56db22":u.genState==="error"?"#c0392b22":"#33333344",color:u.segmentedModel?"#f59e0b":u.model3d?"#a78bfa":u.genState==="done"?"#22c55e":u.genState==="loading"?"#6ea8fe":u.genState==="error"?"#ef4444":"var(--text2)"}}>{u.segState==="loading"?(u.segProgress?u.segProgress+"%":"Seg..."):u.tripoState==="loading"?(u.tripoProgress?u.tripoProgress+"%":"3D..."):u.segmentedModel?"Seg":u.model3d?"3D":u.genState==="done"?"klar":u.genState==="loading"?"...":u.genState==="error"?"fel":"väntar"}</span>
    ...
    <div style={{flex:1,background:"#fadcd9",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
        {!cur?(<p style={{color:"#888",fontSize:"14px"}}>Inga uppladdade bilder</p>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",justifyContent:"center"}}>
            {cur.model3d?(<>
              <button onClick={()=>setTab("3d")} style={B("#22c55e")}>Visa 3D →</button>
              ...
              ):cur.segmentedModel?(<button onClick={()=>setTab("3d")} style={B("#f59e0b")}>Visa segmenterad →</button>
              ):(<button onClick={()=>runSegmentation(selected)} style={B("#f59e0b")}>Segmentera 3D ⇒</button>)}
            ...
            ):cur.tripoState==="error"?(<button onClick={()=>runTripo(selected)} style={B("#ef4444")}>Försök igen →</button>
            ):(<button onClick={()=>runTripo(selected)} style={B("#7c3aed")}>Skapa 3D →</button>)}
          </div>
