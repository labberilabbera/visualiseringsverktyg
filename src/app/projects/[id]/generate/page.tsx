    ...
    <aside style={{width:"200px",minWidth:"200px",borderRight:"1px solid var(--border)",background:"var(--bg2)",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"8px"}}>{uploads.map((u,i)=>(<div key={u.id} onClick={()=>setSelected(i)} style={{marginBottom:"8px",borderRadius:"8px",overflow:"hidden",border:selected===i?"2px solid #1a56db":"1px solid var(--border)",cursor:"pointer",background:"var(--bg)"}}>
        ...
        <div style={{display:"flex",gap:"3px",padding:"4px",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:"9px",padding:"2px 5px",borderRadius:"4px",fontWeight:500,background:u.segTaskId&&u.model3d?"#f59e0b22":u.model3d?"#7c3aed22":u.genState==="done"?"#16a34a22":u.genState==="loading"?"#1a56db22":u.genState==="error"?"#c0392b22":"#33333344",color:u.segTaskId&&u.model3d?"#f59e0b":u.model3d?"#a78bfa":u.genState==="done"?"#22c55e":u.genState==="loading"?"#6ea8fe":u.genState==="error"?"#ef4444":"var(--text2)"}}>{u.tripoState==="loading"?(u.tripoProgress?u.tripoProgress+"%":"3D..."):u.segTaskId&&u.model3d?"Seg klar":u.model3d?"3D klar":u.genState==="done"?"klar":u.genState==="loading"?"...":u.genState==="error"?"fel":"vÃ¤ntar"}</span>
          <div style={{display:"flex",gap:"3px"}}><button onClick={e=>{e.stopPropagation();generateOne(i);}} style={{background:"#1a56db22",border:"none",borderRadius:"4px",padding:"2px 5px",cursor:"pointer",fontSize:"10px",color:"#6ea8fe"}}>âº</button><button onClick={e=>{e.stopPropagation();deleteUpload(i);}} style={{background:"#c0392b22",border:"none",borderRadius:"4px",padding:"2px 5px",cursor:"pointer",fontSize:"10px",color:"#ef4444"}}>â</button></div>
        </div>
      </div>))}</div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
        {!cur?(<p style={{color:"#888",fontSize:"14px"}}>Inga uppladdade bilder</p>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",justifyContent:"center"}}>
            <button onClick={()=>dlUrl(cur.aiImage!,"ai-bild.jpg")} style={B("#555")}>â¬ Ladda ner</button>
            <button onClick={()=>shareOrDl(cur.aiImage!,"ai-bild.jpg","image/jpeg")} style={B("#1a56db")}>â Dela</button>
          </div>
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",justifyContent:"center"}}>
            {cur.model3d?(<button onClick={()=>setTab("3d")} style={B(cur.segTaskId?"#f59e0b":"#22c55e")}>{cur.segTaskId?"Visa segmenterad â":"Visa 3D â"}</button>
            ...
            ):cur.tripoState==="error"?(<button onClick={()=>runTripo(selected,false)} style={B("#ef4444")}>FÃ¶rsÃ¶k igen â</button>
            ):(<><button onClick={()=>runTripo(selected,false)} style={B("#7c3aed")}>Skapa 3D â</button><button onClick={()=>runTripo(selected,true)} style={B("#f59e0b")}>Segmentering â</button></>)}
          </div>
        </div>):cur.genState==="error"?(<div style={{textAlign:"center"}}><p style={{color:"#ef4444",fontSize:"13px",marginBottom:"8px"}}>Fel: {cur.aiError}</p><button onClick={()=>generateOne(selected)} style={B("#1a56db")}>FÃ¶rsÃ¶k igen</button></div>
        ):(<div style={{textAlign:"center"}}><p style={{color:"#888",fontSize:"13px",marginBottom:"8px"}}>Ingen AI-bild Ã¤nnu</p><button onClick={()=>generateOne(selected)} style={B("#1a56db")}>Generera denna</button></div>)
        ):tab==="skiss"?(<SkissView projectId={projectId} upload={cur}/>
        ):(cur.segTaskId?<SegViewer modelUrl={cur.model3d||""} segTaskId={cur.segTaskId} projectId={projectId} uploadId={cur.id}/>:cur.model3d?<ModelViewer modelUrl={cur.model3d} uploadId={cur.id}/>:(<p style={{color:"#888",fontSize:"13px"}}>{cur.aiImage?"Klicka Skapa 3D":"Generera AI-bild fÃ¶rst"}</p>))}
      </div>
    </div>
...
function SkissView({projectId,upload}:{projectId:string;upload:Upload}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"12px",maxWidth:"600px",width:"100%"}}>
    ...
    <div style={{display:"flex",gap:"8px"}}>
      <button onClick={()=>dlUrl(src,"skiss.jpg")} style={{padding:"7px 14px",background:"#555",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>â¬ Ladda ner</button>
      <button onClick={()=>shareOrDl(src,"skiss.jpg","image/jpeg")} style={{padding:"7px 14px",background:"#1a56db",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>â Dela</button>
    </div>
  </div>);}
...
function MRCodeModal({modelUrl,uploadId,onClose}:{modelUrl:string;uploadId:number;onClose:()=>void}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}} onClick={onClose}>
    <div style={{background:"white",borderRadius:"16px",padding:"32px",textAlign:"center",maxWidth:"320px",width:"90%"}} onClick={e=>e.stopPropagation()}>
      ...
      <p style={{fontSize:"13px",color:"#666",marginBottom:"24px"}}>Ãppna Meta Quest Browser och gÃ¥ till:<br/><strong style={{color:"#1a56db"}}>{window.location.origin}/ar</strong></p>
...
function ModelViewer({modelUrl,uploadId}:{modelUrl:string;uploadId:number}){
  ...
  return(<div style={{width:"100%",maxWidth:"600px",display:"flex",flexDirection:"column",gap:"10px"}}>
    <div style={{borderRadius:"12px",overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.15)",background:"#f5e8e5"}}>
      ...
      <p style={{textAlign:"center",fontSize:"11px",color:"#aaa",padding:"6px 0",margin:0}}>Dra fÃ¶r att rotera Â· Scroll fÃ¶r zoom</p>
    </div>
    <div style={{display:"flex",gap:"8px",justifyContent:"center",flexWrap:"wrap"}}>
      <button onClick={()=>dlUrl(proxySrc,"3d-modell.glb")} style={{padding:"7px 14px",background:"#555",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>â¬ Ladda ner GLB</button>
      <button onClick={()=>{if((navigator as any).share)(navigator as any).share({title:"3D",url:window.location.origin+proxySrc}).catch(()=>dlUrl(proxySrc,"3d-modell.glb"));else dlUrl(proxySrc,"3d-modell.glb");}} style={{padding:"7px 14px",background:"#7c3aed",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>â Dela</button>
      <button onClick={()=>setShowMR(true)} style={{padding:"7px 14px",background:"#0ea5e9",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>ð¥½ Visa i MR</button>
    </div>
...
function SegViewer({modelUrl,segTaskId,projectId,uploadId}:{modelUrl:string;segTaskId:string;projectId:string;uploadId:number}){
  ...
  return(<div style={{width:"100%",maxWidth:"600px",display:"flex",flexDirection:"column",gap:"10px"}}>
    ...
    <div style={{background:"white",borderRadius:"10px",padding:"12px",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}}>
      <p style={{margin:"0 0 8px",fontSize:"12px",fontWeight:600,color:"#333"}}>Ãndra en specifik del</p>
      {loadingNames?(<p style={{fontSize:"11px",color:"#aaa",margin:0}}>LÃ¤ser delar...</p>):(
        ...
        {selPart&&!processing&&(<div style={{display:"flex",gap:"8px"}}>
          ...
          <button onClick={applyTexture} disabled={!partPrompt.trim()} style={{padding:"7px 14px",background:"#f59e0b",border:"none",borderRadius:"7px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>Ãndra</button>
        </div>)}
    ...
    <div style={{display:"flex",gap:"8px",justifyContent:"center",flexWrap:"wrap"}}>
      <button onClick={()=>dlUrl(proxySrc,"seg-modell.glb")} style={{padding:"7px 14px",background:"#555",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>â¬ Ladda ner GLB</button>
      <button onClick={()=>setShowMR(true)} style={{padding:"7px 14px",background:"#0ea5e9",border:"none",borderRadius:"8px",fontSize:"12px",fontWeight:500,color:"white",cursor:"pointer"}}>ð¥½ Visa i MR</button>
    </div>
