...
export default function GeneratePage() {
  ...
  async function generateOne(i: number) {
    ...
    try {
      ...
      if (json.images?.[0]) {
        setUploads(prev => prev.map((x, idx) => idx === i ? { ...x, aiImage: json.images[0], genState: "done" } : x));
        setTab("ai");
      } else {
        setUploads(prev => prev.map((x, idx) => idx === i ? { ...x, aiError: json.error || "fel", genState: "error" } : x));
      }
    } catch (e) {
      setUploads(prev => prev.map((x, idx) => idx === i ? { ...x, aiError: String(e), genState: "error" } : x));
    }
  }
  ...
  async function runTripo(i: number) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 60; attempt++) {
        ...
        const poll = await fetch("/api/tripo/generate?taskId=" + taskId);
        const pd = await poll.json();
        if (pd.status === "success" && pd.modelUrl) {
          setUploads(prev => prev.map((x, idx) => idx === i ? { ...x, model3d: pd.modelUrl, tripoState: "done" } : x));
          setTab("3d");
          return;
        }
          ...
          {uploads.map((u, i) => (
            <div key={u.id} onClick={() => setSelected(i)} style={{ marginBottom: "8px", borderRadius: "8px", overflow: "hidden", border: selected === i ? "2px solid #1a56db" : "1px solid var(--border)", cursor: "pointer", background: "var(--bg)" }}>
              <div style={{ height: "72px", background: "#111", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                {u.aiImage ? <img src={u.aiImage} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
      ...
      <div style={{ flex: 1, background: "#fadcd9", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "10px 16px", borderBottom: "1px solid #e8b8b0", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f5cdc8" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: 1, minWidth: 0 }}>
            {editingPrompt ? (
              <input autoFocus value={prompt} onChange={e => setPrompt(e.target.value)} onBlur={() => setEditingPrompt(false)} onKeyDown={e => e.key === "Enter" && setEditingPrompt(false)} style={{ flex: 1, padding: "5px 8px", borderRadius: "6px", border: "1px solid #ccc", fontSize: "12px", background: "white", color: "#111", outline: "none" }} />
            ) : (
          ...
          <div style={{ display: "flex", gap: "6px", flexShrink: 0, marginLeft: "10px" }}>
            ...
            <button onClick={generateAll} disabled={anyLoading || uploads.length === 0} style={{ padding: "6px 14px", background: anyLoading ? "#aaa" : "#1a56db", border: "none", borderRadius: "8px", fontSize: "12px", fontWeight: 500, color: "white", cursor: anyLoading ? "not-allowed" : "pointer" }}>
...
function SkissView({ projectId, upload }: { projectId: string; upload: Upload }) {
  ...
  useEffect(() => {
    fetch("/api/projects/" + projectId + "/uploads/" + upload.id + "/data")
      .then(r => r.json()).then(d => setSrc("data:" + d.mimetype + ";base64," + d.data))
      .catch(() => setSrc(null));
  }, [upload.id, projectId]);
