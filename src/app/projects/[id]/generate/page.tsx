...
type Upload = { id:number;filename:string;mimetype:string;aiImage?:string;aiError?:string;genState:GenState;model3d?:string;tripoState?:string;tripoProgress?:number;tripoTaskId?:string;segmentedModel?:string;segState?:string;segProgress?:number; };
...
async function runSegmentation(i:number){const tripoTaskId=uploadsRef.current[i]?.tripoTaskId;if(!tripoTaskId)return;setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segState:"loading",segProgress:0}:x));const u=uploadsRef.current[i];try{const res=await fetch("/api/tripo/segment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tripoTaskId})});const json=await res.json();if(!json.segTaskId){setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segState:"error"}:x));return;}const segTaskId=json.segTaskId;for(let a=0;a<90;a++){await new Promise(r=>setTimeout(r,4000));const pd=await(await fetch("/api/tripo/segment?segTaskId="+segTaskId)).json();if(pd.status==="success"&&pd.modelUrl){await fetch("/api/projects/"+projectId+"/uploads/"+u.id+"/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({segmentedModelUrl:pd.modelUrl})});setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segmentedModel:pd.modelUrl,segState:"done"}:x));setTab("3d");return;}if(pd.status==="failed"||pd.status==="cancelled"){setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segState:"error"}:x));return;}setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segProgress:pd.progress??0}:x));}setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segState:"error"}:x));}catch{setUploads(prev=>prev.map((x,idx)=>idx===i?{...x,segState:"error"}:x));}}
...
<span style={{fontSize:"9px",padding:"2px 5px",borderRadius:"4px",fontWeight:500,background:u.segmentedModel?"#f59e0b22":u.model3d?"#7c3aed22":u.genState==="done"?"#16a34a22":u.genState==="loading"?"#1a56db22":u.genState==="error"?"#c0392b22":"#33333344",color:u.segmentedModel?"#f59e0b":u.model3d?"#a78bfa":u.genState==="done"?"#22c55e":u.genState==="loading"?"#6ea8fe":u.genState==="error"?"#ef4444":"var(--text2)"}}>{u.segState==="loading"?(u.segProgress?u.segProgress+"%":"Seg..."):u.segmentedModel?"Seg klar":u.tripoState==="loading"?(u.tripoProgress?u.tripoProgress+"%":"3D..."):u.model3d?"3D klar":u.genState==="done"?"klar":u.genState==="loading"?"...":u.genState==="error"?"fel":"vantar"}</span>
...
):cur.model3d?(<button onClick={()=>setTab("3d")} style={btn("#22c55e")}>Visa 3D &#8594;</button>
...
):cur.tripoState==="error"?(<button onClick={()=>runTripo(selected)} style={btn("#ef4444")}>Forsok igen &#8594;</button>
):(<button onClick={()=>runTripo(selected)} style={btn("#7c3aed")}>Skapa 3D &#8594;</button>)}
{cur.model3d&&!cur.segmentedModel&&cur.segState!=="loading"&&(<button onClick={()=>runSegmentation(selected)} style={btn("#f59e0b")}>Segmentera 3D &#8680;</button>)}
{cur.segState==="loading"&&(<div style={{display:"flex",alignItems:"center",gap:"8px"}}><div style={{width:"120px",height:"5px",background:"#ddd",borderRadius:"3px",overflow:"hidden"}}><div style={{width:(cur.segProgress||0)+"%",height:"100%",background:"#f59e0b",transition:"width 0.5s"}}/></div><p style={{color:"#f59e0b",fontSize:"11px",margin:0}}>{cur.segProgress||0}% seg...</p></div>)}
{cur.segState==="error"&&(<button onClick={()=>runSegmentation(selected)} style={btn("#ef4444")}>Forsok segmentera igen</button>)}
</div>
</div>):cur.genState==="error"?(<div style={{textAlign:"center"}}><p style={{color:"#ef4444",fontSize:"13px",marginBottom:"8px"}}>Fel: {cur.aiError}</p><button onClick={()=>generateOne(selected)} style={btn("#1a56db")}>Forsok igen</button></div>
):(<div style={{textAlign:"center"}}><p style={{color:"#888",fontSize:"13px",marginBottom:"8px"}}>Ingen AI-bild annu</p><button onClick={()=>generateOne(selected)} style={btn("#1a56db")}>Generera denna</button></div>)
):tab==="skiss"?(<SkissView projectId={projectId} upload={cur}/>
):(cur.segmentedModel?<ModelViewer modelUrl={cur.segmentedModel} label="Segmenterad 3D"/>:cur.model3d?<ModelViewer modelUrl={cur.model3d} label="3D-modell"/>:(<p style={{color:"#888",fontSize:"13px"}}>{cur.aiImage?"Klicka Skapa 3D under AI-bilden":"Generera AI-bild forst"}</p>))}
</div>
</div>
...
function ModelViewer({modelUrl,label}:{modelUrl:string;label?:string}){
...
{label&&<p style={{textAlign:"center",fontSize:"11px",color:"#888",margin:0,fontWeight:500}}>{label}</p>}
