...
export default function ProjectsPage() {
  ...
  const inp = { width: "100%", padding: "0.625rem 0.75rem", background: "var(--bg)", border: "1px solid var(--border2)", borderRadius: "8px", color: "var(--text)", fontSize: "0.875rem", outline: "none", boxSizing: "border-box" as const };
  const bp = { padding: "0.5rem 1rem", background: "var(--accent)", color: "white", border: "none", borderRadius: "8px", fontSize: "0.875rem", fontWeight: 500, cursor: "pointer" };
    ...
    <main style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "system-ui, sans-serif", color: "var(--text)", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, padding: "2rem", maxWidth: "600px", margin: "0 auto", width: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="8" height="8" rx="2" fill="white"/><rect x="13" y="3" width="8" height="8" rx="2" fill="white" opacity="0.6"/><rect x="3" y="13" width="8" height="8" rx="2" fill="white" opacity="0.6"/><rect x="13" y="13" width="8" height="8" rx="2" fill="white" opacity="0.3"/></svg>
            </div>
        ...
        {showNew && (
          <div style={{ ...card, padding: "1.25rem", marginBottom: "1rem" }}>
            ...
            <input autoFocus value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && createProject()} placeholder="T.ex. Trähantverk" style={{ ...inp, marginBottom: "0.75rem" }}/>
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
              <button onClick={() => { setShowNew(false); setNewName(""); }} style={bg}>Avbryt</button>
              <button onClick={createProject} style={bp}>Skapa</button>
            </div>
          </div>
        )}
          ...
          {projects.map(proj => (
            <div key={proj.id} style={{ ...card, padding: "1rem 1.25rem", display: "flex", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                ...
                <button onClick={() => setConfirmDelete(proj)} style={{ background: "transparent", border: "1px solid #5a2020", borderRadius: "6px", padding: "0.25rem 0.5rem", cursor: "pointer", display: "flex" }}>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 3.5h10M5.5 3.5V2.5h3v1M4.5 3.5l.5 8h4l.5-8" stroke="#c44" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </button>
              </div>
            </div>
          ))}
      ...
      {confirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem", zIndex: 50 }}>
          <div style={{ ...card, padding: "1.5rem", maxWidth: "340px", width: "100%" }}>
            ...
            <p style={{ color: "var(--text2)", fontSize: "0.875rem", margin: "0 0 1.25rem" }}>Är du säker på att du vill radera <strong style={{ color: "var(--text)" }}>{confirmDelete.name}</strong>?</p>
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
              <button onClick={() => setConfirmDelete(null)} style={bg}>Avbryt</button>
              <button onClick={deleteProject} style={{ ...bp, background: "#c0392b" }}>Ja, radera</button>
            </div>
          </div>
        </div>
      )}
      <div style={{ padding: "1rem 2rem", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
          {isAdmin && <button onClick={() => router.push("/admin")} style={{ ...bg, fontSize: "0.75rem", borderColor: "#1a56db", color: "#6ea8fe" }}>Admin</button>}
