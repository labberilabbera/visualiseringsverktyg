    ...
    <main style={s.main}>
      <div style={{ maxWidth: "640px", margin: "0 auto" }}>
        ...
        {showAdd && (
          <div style={{ ...s.card, padding: "1.25rem", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              <div><label style={{ fontSize: "0.75rem", color: "var(--text2)", display: "block", marginBottom: "0.375rem" }}>Lösenord</label><input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="••••••••" style={s.input}/></div>
              <div><label style={{ fontSize: "0.75rem", color: "var(--text2)", display: "block", marginBottom: "0.375rem" }}>Roll</label>
                <select value={newRole} onChange={e => setNewRole(e.target.value)} style={{ ...s.input, cursor: "pointer" }}>
                  <option value="user">Användare</option><option value="admin">Admin</option>
                </select>
              </div>
        ...
        <div style={{ ...s.card, overflow: "hidden" }}>
          ...
          {users.length === 0 && <div style={{ padding: "2rem", textAlign: "center", color: "var(--text3)", fontSize: "0.875rem" }}>Inga användare</div>}
          {users.map((user, i) => (
            <div key={user.id} style={{ padding: "0.875rem 1.25rem", borderBottom: i < users.length - 1 ? "1px solid var(--border)" : "none", display: "grid", gridTemplateColumns: "1fr auto auto", gap: "1rem", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "0.375rem", width: "140px" }}>
                <button onClick={() => { setEditUser(user); setEditPassword(""); }} style={{ ...s.btnG, fontSize: "0.75rem", padding: "0.3rem 0.6rem" }}>Lösenord</button>
      ...
      {confirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem", zIndex: 50 }}>
          <div style={{ ...s.card, padding: "1.5rem", maxWidth: "340px", width: "100%" }}>
            ...
            <p style={{ color: "var(--text2)", fontSize: "0.875rem", margin: "0 0 1.25rem" }}>Är du säker på att du vill radera <strong style={{ color: "var(--text)" }}>{confirmDelete.email}</strong>?</p>
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
